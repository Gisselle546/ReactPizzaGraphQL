...
function completeTotal(total){
 return parseFloat(tax(total.toFixed(2)))+parseFloat(4.00)+parseFloat(total.toFixed(2));
    ...
    return(
      ...
      square={true}
    >
       <Button size="large" endIcon={<ArrowForwardIcon/>} className={classes.checkoutbutton}>
          Checkout
        </Button>
                <Typography variant="h6">Subtotal: ${total.toFixed(2)}</Typography>
                <Typography variant="h6">Delivery Fee: $4.00</Typography>
                <Typography variant="h6">Tax: ${tax(total)}</Typography>
                <Typography variant="h6"><em>Total: ${completeTotal(total)}</em></Typography>
